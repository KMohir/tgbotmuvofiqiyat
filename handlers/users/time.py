...
async def cmd_time(message: types.Message, state: FSMContext):
    ...
    print(f"Пользователь {message.from_user.id} вызвал команду /time")
    ...
    print(f"Состояние установлено: {current_state} для пользователя {message.from_user.id}")


@dp.message_handler(lambda message: not message.text.startswith('/'), state=TimeSelection.time)
async def process_time(message: types.Message, state: FSMContext):
    print(f"Хендлер process_time вызван для пользователя {message.from_user.id}")
    time_input = message.text
    print(f"Выбранное время: {time_input}")

    if time_input == "Orqaga qaytish":
        await message.answer("Vaqt tanlash bekor qilindi.", reply_markup=ReplyKeyboardRemove())
        await state.finish()
        print(f"Состояние завершено (отмена) для пользователя {message.from_user.id}")
        return
    ...
    try:
        ...
        if not (0 <= hours <= 23 and 0 <= minutes <= 59):
            ...
            print(f"Неверный формат времени: {time_input}")
            return
    except ValueError:
        await message.answer(
            "Noto'g'ri format! Iltimos, quyidagi tugmalardan birini tanlang:",
        ...
        print(f"Ошибка формата времени: {time_input}")
        return
    ...
    await state.finish()
    print(f"Время {time_input} сохранено для пользователя {message.from_user.id}, состояние завершено")